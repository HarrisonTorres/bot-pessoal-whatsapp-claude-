...
- **bridge**: conecta como dispositivo vinculado. Processa só grupos presentes em `config/groups.json`, atendendo qualquer membro deles (ou apenas o dono, nos grupos com `somenteDono`). Ignora as próprias respostas (evita loop). Baixa mídia para `data/media/<grupo>/`. Mantém uma fila durável e executa **uma chamada do Claude por vez no total** (uso pessoal; protege a cota). Envia texto e qualquer arquivo novo que apareça em `data/outbox/<grupo>/`. O remetente é lido em `key.participant` ou `key.participantAlt` (o Baileys 7 preenche o gêmeo LID/telefone) normalizado. Cada prompt começa com `[Mensagem <id> recebida em <data>]`, e um agente usa esse id como chave de deduplicação. Uma trava por `dataDir` (`data/bridge.lock`) impede duas instâncias da ponte disputarem a mesma sessão do WhatsApp — achado ao vivo em 2026-09-21: duas rodando juntas se derrubam uma à outra em loop (code 440).
